            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        {{ __('Create Country') }}
                        <a href="{{ route('unit-ranks.indicator-categories.index',$rank_id,$id) }}" class="float-right">Back</a>
                    </div>

                    <div class="card-body">
                        <form method="POST" action="{{ route('unit-ranks.indicator-categories.edit',[$rank_id, $id]) }}">
                            @csrf
                            @method('PUT')
                ...
                <div class="m-2 p-2">
                    <form method="POST" action="{{ route('unit-ranks.indicator-categories.destroy',[$rank_id ,$id]) }}">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-danger">Delete {{ $indicator->name }}</button>
                    </form>
                </div>
            </div>
        </div>
